...
The research rule carries one clause I would put in every setup where an agent writes tests, because "make the tests pass" and "check the code is correct" are different goals that look identical from outside, and the first is much easier. Never weaken a test to reach green: a failing test is a finding to report, not a tolerance to loosen or a skip to add. Expected values come from theory, a reference implementation, or a property that must hold, never from recording what the code currently outputs. And every new check ships with a planted defect proving it can fail. The [companion post](/blogs/research-template/index/) shows what that looks like in a real suite.
...
1. **A knowledge map** (`.claude/knowledge-map.json`): a list of `{match, doc, nudge}` entries connecting file paths to the docs that describe them. Usually the paths are code (migrations pointing at a schema skill, data loaders pointing at a datasets doc), but any path works: my maps also watch eval outputs (nudging an experiment-log entry) and even Claude's own memory folder (more on that in the memory section). The map is not a one-time config: it starts with a handful of entries at setup and grows on the go. Whenever a doc starts covering a new area of code, a new entry gets added, and the repo's CLAUDE.md instructs Claude to do that itself, so the map keeps pace with the project instead of becoming a drifted doc about drift.
...
- **Against the silo**: the drift map's bridge entry. The drift hook exists for code-to-docs sync, but its mechanism is generic: it fires after *every* file write and matches the written path against regexes. A memory save is an ordinary file write, so one map entry whose regex matches the memory directory (instead of a code path) catches it and fires a nudge: "if this fact is shareable and non-secret, promote it to CLAUDE.md or a skill." Note the division of labor: the regex only detects *that* a memory was written, content-unseen; the shareable-vs-secret judgment is Claude's, guided by criteria carried in the nudge text itself; and the promotion is an ordinary committed edit you see in the diff. Mechanism detects, model judges, human confirms. Private learnings become versioned team knowledge deliberately, not never.
...
- **A periodic consolidation pass.** Update-on-conflict only works when the conflict gets noticed, so every so often I run `/consolidate-memory` over a project: it merges near-duplicates, fixes or deletes stale facts, and prunes the index. Worth knowing that this one ships with Claude Code rather than being something you have to write, which is why it is not in my repo. It is the memory-tier analog of the docs audit script: the deliberate sweep that catches what the automatic behavior missed.
...
- **Promotion out of the drift zone.** The most important counter is structural, and it is honest to say it does not *stop* memory drift at all: it makes memory drift not matter. Facts that must *stay* true don't belong in memory; the silo nudge pushes them up into CLAUDE.md or a skill, converting a memory-drift problem into a docs-drift problem, which is the one the hook and audit actually solve. The three counters above slow the drift of what remains; this one moves everything important out of the blast radius. Memory drift is tolerable precisely because memory is the scratch tier.
